...
export default class OtherEvent extends Component {
    ...
    render() {
        ...
        const lecturersName = cellEventObj.speakersInfo.map(speakerObj =>
            (
              <p key={speakerObj.id} className="event-lector">{speakerObj.name}</p>
            ));
        return (
          <div className="event-box">
            ...
            <div
              className={`event-descr-item place ${
            ...
            <div
              className={`event-descr-item useful-links-ctn ${
                    eventType === ('lecture') ? ' ' :
                    (eventType === ('webinar') ? ' ' : 'none')}`}
            >
